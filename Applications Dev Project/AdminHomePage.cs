...
namespace Applications_Dev_Project
{
    public partial class AdminHomePage : Form
    {
        ...
        private void UpdateUI()
        {
            ...
            dgvUsers.DataSource = DataStore.Users.AllUsers;
            dgvUsers.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            dgvUsers.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.AllCells;

            var helpRequests = DataStore.Contacts.AllContacts
                                   .Select(contact => 
                                   { 
                                       var sender = DataStore.Users.AllUsers.FirstOrDefault(user => user.ID == contact.SenderID);
                                       return new
                                       {
                                           contact.ID,
                                           contact.TimeSent,
                                   ...
                                   .OrderByDescending(c => c.IsResolved)
                                   .ToList();

            dgvContacts.DataSource = null;
            dgvContacts.DataSource = helpRequests;
            ...
            dgvContacts.Columns["Message"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
        ...
        {
            if (dgvUsers.SelectedRows.Count == 0)   //Error message if no user is selected
            {
            ...
            if (editUser.DialogResult == DialogResult.OK)
                UpdateUI();
        }

        private void btnMessage_Click(object sender, EventArgs e)
        {
            Contact contactToView = DataStore.Contacts.AllContacts
            .FirstOrDefault(contact => contact.ID == (Guid)dgvContacts.CurrentRow.Cells["ID"].Value);

            ContactSupport contactSupport = new ContactSupport(contactToView);
            Hide();
